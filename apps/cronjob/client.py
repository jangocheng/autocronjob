    ...
    def execute(self, command, parameter):
        ...
        def run(*args):
            ...
            return {'status': 'success',
                    'message': 'Task %s has been assigned task id is %s.' % (task_name, response.id)}
        ...
        def rate_limit(*args):
            ...
            if len(response) != 0 and response[0][destination].__contains__('ok'):
                response = {'status': 'success', 'message': '%s' % (response[0][destination]['ok'])}
            elif len(response) != 0 and response[0][destination].__contains__('error'):
        ...
        def poolgrow(*args):
            ...
            response = ctrl.pool_grow(n=grown_num, reply=True,
                                      destination=[destination])
            if len(response) != 0 and response[0][destination].__contains__('ok') and 'pool will grow' in \
        ...
        def poolshrink(*args):
            ...
            response = ctrl.pool_shrink(n=shrink_num, reply=True,
                                        destination=[destination])
            if len(response) != 0 and response[0][destination].__contains__(
                ...
                response = {'status': 'failure',
                            'message': 'Can\'t shrink pool. All processes busy on server %s! Please try this later.' % (
                                destination)}
            elif len(response) != 0 and response[0][destination].__contains__('ok') and 'pool will shrink' in \
                    response[0][destination]['ok']:
                response = {'status': 'success',
